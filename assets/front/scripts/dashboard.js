(function($) {
  $(function() {
    ...
    function editSocial() {
      ...
      let editingSocial = SM_USER_SOCIALS.find(social=> social[social['social_name'] + '-url'] === url);
      
      if(!editingSocial) {
        let data = {title: SM_DATA.i18n.social_not_found_title, message: SM_DATA.i18n.social_not_found_message};
        swalError({data});
        return;
      }
      ...
      let fieldset   = $(`.submit-socials-wrapper fieldset.${socialName}`).filter(function() {
        return $(this).data('social-url').trim() === url.trim()
      })
      
      fieldset.find('input').val(socialUrl)
      $(`.submit-socials-wrapper .form-group.description textarea`).val(socialdesc)
    ...
    function deleteSocial() {
      ...
      let url             = $this.closest('.social').data('social-url')
      let userId          = $('.user-socials').data('user-id')
      let deletingSocial  = SM_USER_SOCIALS.find(social=> social[social['social_name'] + '-url'] === url);
      ...
      .then(result=> {
        ...
        $.ajax({
          ...
          beforeSend: () => $('body').append('<div class="loader"><div></div></div>'),
          success: (res)=> {
            Swal.fire({
    ...
    $('#submit-socials').on('submit', function (e) {
      ...
      data = data.filter(field=> {
        return !field.name.includes('url') || (field.name.includes('url') && field.value.trim() === $form.data('social-url').trim())
      })

      data = [...data, {name: 'social[social_name]', value: $form.data('social-name').trim()}];
